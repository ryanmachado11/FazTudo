...
function buildDocumentFingerprint(userId, email, documentNumber) {
    const source = documentNumber?.trim() || `${userId}:${email}`;
    return createHash('sha256').update(source).digest('hex');
}
async function storeDataUrlImage(userId, label, input) {
    if (!input) {
        return null;
    }
            ...
            const verification = await prisma.$transaction(async (tx) => {
                const createdVerification = await tx.userVerification.create({
                    data: {
                        ...
                        documentNumberEncrypted: data.documentNumberEncrypted || buildDocumentFingerprint(user.sub, account.email, data.documentNumber),
