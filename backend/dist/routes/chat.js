...
const optionalUuid = z.preprocess((value) => (value === null || value === '' ? undefined : value), z.string().uuid().optional());
...
function chatError(statusCode, message) {
...
async function findRoomDetails(id) {
    return prisma.chatRoom.findUnique({ where: { id }, include: roomDetails });
}
async function getOrCreateRoomForUser(user, payload) {
    ...
    if (serviceRequestId) {
        ...
        if (!providerId || providerId === user.sub)
        ...
        if (!payload.clientId || payload.clientId === user.sub)
